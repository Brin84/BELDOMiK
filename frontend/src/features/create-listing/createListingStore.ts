  ...
  submitForModeration: async () => {
    ...
    try {
      ...
      const backendData = {
        ...
        metro_station_id: formData.metro_station_id,
        metro_distance: formData.metro_distance,
        ...
        balcony_count: formData.balcony_count,
        loggia_count: formData.loggia_count,
        parking: formData.has_parking,
        elevator: formData.has_elevator,
        is_new_building: formData.is_new_building || false,
  ...
  loadDraft: async (id: number) => {
    ...
    try {
      ...
      const draftData: PropertyCreate = {
        ...
        // Дополнительно: восстанавливаем метро, площади, балконы, лоджии, новостройку.
        // Старые объявления без balcony_count, но с balcony=true → показываем «1 балкон».
        balcony_count: response.balcony_count ?? (response.balcony ? 1 : undefined),
        metro_station_id: response.metro_station_id ?? undefined,
        metro_distance: response.metro_distance ?? undefined,
        living_area: response.living_area ?? undefined,
        kitchen_area: response.kitchen_area ?? undefined,
        loggia_count: response.loggia_count ?? undefined,
        is_new_building: response.is_new_building || false,
      };

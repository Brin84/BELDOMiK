...
const DEFAULT_CENTER: [number, number] = [27.5618, 53.9022]; // Minsk center
...
export function MapPage() {
  ...
  const [selectedProperty, setSelectedProperty] = useState<import('@/shared/api/types').PropertyShort | null>(null);
  ...
  // Load districts, neighborhoods, streets, metro when city changes
  useEffect(() => {
    if (filters.city_id) {
  ...
  // Update propertiesStore filters
  ...
  // Extract unique renovation types
  ...
  const handlePropertySelect = useCallback((property: import('@/shared/api/types').PropertyShort) => {
    trigger('selection');
      ...
      (err) => {
        ...
        switch (err.code) {
          case err.PERMISSION_DENIED:
            message = 'Доступ к геолокации запрещен. Включите разрешение в настройках браузера.';
            break;
          case err.POSITION_UNAVAILABLE:
            message = 'Геолокация недоступна. Попробуйте позже.';
            break;
          case err.TIMEOUT:
  ...
  // Render
  return (
    <div className="relative flex flex-col" style={{ height: 'calc(var(--tg-viewport-stable-height, 100vh))' }}>
      {/* Map View */}
      {viewMode === 'map' && (
        <div className="flex-1 relative overflow-hidden">
          ...
          <MapControls
            ...
            onFiltersClick={() => setFilterBottomSheetOpen(true)}
          ...
          {/* Active Filter Chips (only on map view) */}
          {hasActiveFilters && (
            <div className="absolute top-4 left-4 right-4 z-20 px-4">
              <ActiveFilterChips
                filters={filters}
                onRemoveFilter={(key) => {
                  trigger('light');
